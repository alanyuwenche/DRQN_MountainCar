...
max_epi_num = 10000 #Replay-Buffer Size(deque)
max_epi_len = 300 #每個epoch最大長度(random update)


# Create Q functions
Q = Q_net(state_space=num_inputs, action_space=num_actions).to(device)
Q_target = Q_net(state_space=num_inputs, action_space=num_actions).to(device)
...
rewards = deque(maxlen=30) #ori: 30
avgR = -200
maxR = -200
epsilon = eps_start
for i in range(episodes):
    ...
    if len(rewards) == 30: #ori: 30
      rew_arr = np.array(rewards)
      meanR = np.mean(rew_arr)
      maxB = np.max(rew_arr)
      if maxB > maxR:
        maxR = maxB
      if meanR > avgR:
      ...
      if i%50 == 0:
        print('i: ',i,' meanR: ',meanR,' avgR: ',avgR,' maxR: ',maxR,' epsilon: ',epsilon)